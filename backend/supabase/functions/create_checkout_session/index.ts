...
serve(async (req) => {
  try {
    ...
    const query = new URLSearchParams({
      select: "id,total_cents,platform_fee_cents,customer_id",
      id: `eq.${order_id}`,
    ...
    const orderResponse = await fetch(
      `${supabaseUrl}/rest/v1/orders?${query.toString()}`,
      {
        method: "GET",
        headers: {
    ...
    if (!amountCents || amountCents <= 0) {
      return new Response(
        JSON.stringify({ error: "Order amount is invalid" }),
        {
          status: 400,
          headers: jsonHeaders,
        },
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url:
        success_url || `${Deno.env.get("APP_URL")}/order/${order_id}/success`,
      cancel_url:
        cancel_url || `${Deno.env.get("APP_URL")}/order/${order_id}/cancel`,
      metadata: {
        order_id,
      },
    });
